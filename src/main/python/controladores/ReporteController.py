...
class ReporteController(QtWidgets.QWidget, ControllerModel):
	...
	def __init__(self, mainWindow, reporteModel=None):
		...
		self.csvHeaders = ["nombreExaminado","id","fecha","genero","edad","fechaNacimiento",
			...
			'TA', 'TR', 'T', 'I', 'C', 'DI', 'DD', 'ABS', 'B', 'A', 'Dif', 'T', 'MVCt', 'MVC', 'DVt', 'DV', 'A', 'P','Ira','AgFis','AgVer','Hos']
		self.escalares = []
		self.escalaresLabel = []
		self.loadReporte()
	...
	def createReporte(self):
		...
		for pruebaName in pruebasRegistradas.keys():
			...
			if pruebaName != 'BussYPerry' and pruebaName != 'SCL-90' and pruebaName != 'Motivos Deportivos de Butt' and pruebaName != 'PSQI':
				#print(pruebaName)
